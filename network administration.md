...
- Коммутатор(свитч) – это небольшое аппаратное устройство, которое объединяет несколько компьютеров в одну локальную сеть (LAN) и работает на уровне 2 модели OSI(Уровень канальный L2 - mac).
- Маршрутизаторы(роутер) – это физические устройства, которые объединяют несколько проводных или беспроводных сетей вместе и работают на третьем уровне модели OSI(Уровень сети L3 - ip).
